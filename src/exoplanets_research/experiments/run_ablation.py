...
from exoplanets_research.validation.baselines import hz_radius_baseline, rank_correlation_summary, top_k_jaccard
...
def build_ranked_candidates(input_path: Path, *, hz_model: str) -> pd.DataFrame:
    ...
    return score_candidates(featured).reset_index(drop=True)
...
def run_baseline_comparison(manifest: dict, output_dir: Path) -> tuple[pd.DataFrame, list[Path]]:
    ...
    baseline = assign_rank(hz_radius_baseline(ranked), "baseline_rank")
...
| --- | --- | --- | --- |
| HWO ExEP Precursor Science Stars | NASA Exoplanet Archive TAP table `{external_config.get("table_name", HWO_EXEP_TABLE)}` | `{external_config.get("path", "data/external/hwo_exep_2023.csv")}` | Mission-facing stellar target list used to test whether ECTP prioritizes known direct-imaging target hosts. |
...
def main() -> None:
    ...
    artifacts = write_paper_artifacts(manifest, [*hz_artifacts, *baseline_artifacts, *external_artifacts, inventory])
